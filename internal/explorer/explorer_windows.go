...
func (e *Explorer) importFiles(extensions ...string) ([]io.ReadCloser, error) {
	...
	if len(paths) == 1 {
		// A single selection stores the complete file path; there is no
		// leading directory element. Treating it as a directory would yield
		// zero files and report a spurious ErrUserDecline.
		filePaths = paths
	} else {
		// Multiple selections store the directory first, then the filenames.
		dir := paths[0]
